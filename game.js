...
let STATE = { X: 0, Y: 0, _X: 0, _Y: 0, SPEED: 2, MODE: 1, SCORE: 0, LEVEL: 1, NEXT: 10, ACTUAL_SCORE: 0, COLOR: '#fff' } // MODE: 1=Playing, 0=Lost
...
$(document).ready(() => {
  ...
  $(document).keydown(e => {
    let key = e.which;
    if(Object.values(KEYS).indexOf(key) != -1)e.preventDefault();
  ...
  function loop() {
    ...
    if (--STARS == 0) {
      STARS = 10;
      let x = Math.floor(Math.random() * (SCREEN_WIDTH - 100) + 50);
      let s = Math.ceil(Math.random() * 5+3);
      let obj = '<span class="active dot" style="color:white" speed="' + s + '"></span>';
      $('body').prepend(obj);
      $('.active').css({ left: x }).removeClass('active');
    }
  ...
  $('#yes').click(() => {
    ...
    $('#gameover').hide();
    STATE = { X: 0, Y: 0, _X: 0, _Y: 0, SPEED: 2, MODE: 1, SCORE: 0, LEVEL: 1, NEXT: 10, ACTUAL_SCORE: 0, COLOR: '#fff' } // MODE: 1=Playing, 0=Lost
  ...
  function getScores() {
    ...
    var url = 'http://54.244.80.189:8080/';
    var method = 'GET';
